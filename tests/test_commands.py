...
def test_parse_tuple_from_string(tup: str, verifier: Callable[[str], bool]):
...
def test_when_generating_random_file_name_with_extension_given_valid_arguments_should_succeed(
    file_meta: FileMeta
):
...
def test_get_file_paths(
    ...
    for subdir_count in range(max_subdir_count + 1):
        ...
        allowed_file_paths = [
            os.path.join(dir_path, fne) for fne in allowed_file_names_with_extension
        ]
...
def test_when_creating_or_updating_file_given_file_not_exists_should_create_file(
...
def test_when_creating_or_updating_file_given_file_exists_should_update_file(
    tmpdir_factory, file_meta: FileMeta
):
    tmp_dir_path = os.path.join(str(tmpdir_factory.getbasetemp()), file_meta.dir_path)
    tmp_file_path = os.path.join(tmp_dir_path, file_meta.name_with_extension)

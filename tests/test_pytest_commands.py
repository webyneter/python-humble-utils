...
def test_when_generating_tmp_file_path_given_relative_tmp_dir_path_should_succeed(
    ...
    actual = generate_tmp_file_path(
        tmpdir_factory, file_meta.name_with_extension, file_meta.dir_path
    )

    assert actual == expected


def test_when_generating_tmp_file_path_given_abs_tmp_dir_path_should_raise(
    tmpdir_factory, file_meta: FileMeta
):
